...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        myNodes = []
        cur_state_nodes = self.s_levels[level] 
        for action in self.all_actions:
            # 1. determine what actions to add and create those PgNode_a objects
            node = PgNode_a(action)
            # we must check that precondition occur at the previous level
            if node.prenodes.issubset(cur_state_nodes):
                # add the node to the list
                myNodes.append(node)
                for each_node in cur_state_nodes:
                    # set the childern of each node to be reaseable node
                    each_node.children.add(node)
                    # set the reachable node to be a parent of  node in the preceding level
                    node.parents.add(each_node)
        
        # create an action level to reachable nodes            
        self.a_levels.append(myNodes)
    ...
    def add_literal_level(self, level):
        ...
        myNodes = set()
        prev_level = self.a_levels[level - 1]
        for node in prev_level:
            # add the nodes to the level state that have an effect on it
            for each_node in node.effnodes:
                
                # add the node to the list
                myNodes.add(each_node)
                # set the parent of effect node to be the preceding level
                each_node.parents.add(node)
                # set the childern of preceing node to be effect node
                node.children.add(each_node)
        
        # create an action level to effected nodes            
        self.s_levels.append(myNodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # make sure that their is no conflict between + of node_a1 and - of node_a2 
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # make sure that their is no conflict between + of node_a1 and - of node_a2 (preconditon)
        # make sure that their is no conflict between - of node_a1 (preconditon) and + of node_a2 
        for effect in node_a1.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # ensure that preconditions of node_a1 are not mutually exclusive with preconditions of node_a2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # check the polarity of node_s1 state and node_s2 state 
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # ensure that preconditions of node_s1 are not mutually exclusive with preconditions of node_s2
